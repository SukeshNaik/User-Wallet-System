...
import com.example.walletSystem.comparator.BankTransactionSortingComparator;
...
public class WalletServiceImpl implements WalletService{
    ...
    public Wallet createWallet(Integer customerId) throws CustomerDoesNotExists, CustomerAlreadyHasWalletException {
        ...
        User c = customerRepository.findById(customerId).orElse(null);
        ...
        Wallet w = new Wallet();
        int accNo=c.getUserAccounts().get(0).getAccountNumber();
        Account a=accountRepository.findAccountByNumber(accNo).get(0);
        w.setUser(c);
        if (c.getUserAccounts()!=null && !c.getUserAccounts().isEmpty()) {
            w.setAccountsInWallet(new ArrayList<>(c.getUserAccounts()));
            //a.setWallet(w);
            a.setWallet(w);
    ...
    public List<Transactions> getStatement(Integer walletId, Integer accountId, Integer n) throws WalletIdDoesNotExistException,
        ...
        List<Transactions> bankTransactions = associateAccount.get(0).getTransactions();
